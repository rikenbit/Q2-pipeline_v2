...
seed = 1234
read_number_mega = 768
read_number = read_number_mega * 100000
ds_id = "ds" + str(read_number_mega) + "M"
ds_rate = [x for x in range(5, 105, 5)]
job_count = 0
...
for i, input_file in enumerate(input_files):
    ...
    for R in ["R1", "R2"]:
        for rate in ds_rate:
            ...
            out_file = os.path.join(output_dir, sample_name + "_" + ds_id +
                                    "_frac" + str(rate).zfill(3) + "_" + R + "_001.fastq.gz")
            sh_file = os.path.join(output_dir,
                                   sample_name + "." + ds_id + ".frac" + str(rate).zfill(3) + "." + R + ".sh")
            stdout_file = os.path.join(output_dir,
                                       sample_name + "." + ds_id + ".frac" + str(rate).zfill(3) + "." + R + ".o.txt")
            stderror_file = os.path.join(output_dir,
                                         sample_name + "." + ds_id + ".frac" + str(rate).zfill(3) + "." + R + ".e.txt")
            ...
            cmd = ("seqtk sample -s" + str(seed) + " " + in_file + " " + str(read_number) + " | gzip - > " + out_file + "\n")
