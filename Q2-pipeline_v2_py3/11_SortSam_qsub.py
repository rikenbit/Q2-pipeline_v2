...
base_dir = os.getcwd()
input_dir = os.path.join(base_dir, "results", "10_STAR")
output_dir = os.path.join(base_dir, "results", "11_SortSam")

if not os.path.isdir(output_dir):
    os.makedirs(output_dir)

input_files = glob.glob(os.path.join(input_dir, "*Aligned.out.sam"))
list_sh_file = []

threads_num = obj['THREADNUM']

for i, input_file in enumerate(input_files):
    sample_name = (os.path.split(input_file)[1].
                   split(".")[0].replace("_STAR", ""))
    output_file = os.path.join(output_dir, sample_name + "_sorted.bam")
    stdout_file = os.path.join(output_dir, sample_name + ".o.txt")
    stderror_file = os.path.join(output_dir, sample_name + ".e.txt")

    job_name = "SortSa_" + str(i)
    ...
    cmd = ("java -XX:ParallelGCThreads=" + str(threads_num) + " -Xmx128g \\\n" +
           ...
           "-jar /usr/local/bin/picard.jar SortSam \\\n" +
           "I=" + input_file + " \\\n" +
           "O=" + output_file + " \\\n" +
           "SO=queryname \\\n" +
           "TMP_DIR=" + output_dir + " \\\n" +
    ...
    sh_file = os.path.join(output_dir, sample_name + ".SortSam.sh")
